...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    {
      // declaration
      char dummy1[sizeof (Declaration*)];

      // declaration_list
      char dummy2[sizeof (DeclarationList*)];
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_root: // root
        value.move< Root* > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
    /// Non copyable.
    PlnParser (const PlnParser&) = delete;
    /// Non copyable.
    PlnParser& operator= (const PlnParser&) = delete;
#endif
...
#if YY_CPLUSPLUS < 201103L
    /// Non copyable.
    PlnParser (const PlnParser&);
    /// Non copyable.
    PlnParser& operator= (const PlnParser&);
#endif
